-- Team Budget System Migration (Safe Version)
-- This migration handles dependencies properly

-- Step 1: Drop dependent objects first
DROP VIEW IF EXISTS member_spending_view CASCADE;
DROP FUNCTION IF EXISTS get_member_summary() CASCADE;

-- Step 2: Add budget column to teams table
ALTER TABLE teams ADD COLUMN IF NOT EXISTS budget DECIMAL DEFAULT NULL;

-- Step 3: Preserve existing member budget data
...
-- Step 4: Now safely drop the budget column from members
ALTER TABLE members DROP COLUMN IF EXISTS budget;

-- Step 5: Create the new get_member_summary function
...
-- Step 6: Recreate the member_spending_view
CREATE VIEW member_spending_view AS
SELECT * FROM get_member_summary();

-- Step 7: Create team summary function
